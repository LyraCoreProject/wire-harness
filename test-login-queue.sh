...
#
# THIS SCRIPT DOES NOT (RE)START THE GATEWAY OR SET GW_MAX_SESSIONS. The queue only engages if the
# gateway currently running was launched with a small GW_MAX_SESSIONS (< N) — see
...
#            validation is the separate 300-login / GW_MAX_SESSIONS=150 run in the operator's
...
#   1. Launch the gateway with GW_MAX_SESSIONS=2 (so N=5 forces 3 of the 5 to queue) — see
#      docs/danger-zones.md §3 for the rest of the launch recipe (GW_SHARD_MAP etc. still apply).
#   2. Run this script.
...
GW="$REPO_ROOT/target/debug/gateway"
PREFIX="LQ"
...
if [[ ! -x "$GW" ]]; then
  ...
  cargo build -p spacetime-core-gateway --manifest-path "$REPO_ROOT/Cargo.toml" 2>&1
fi

: "${GW_COORDINATOR_TOKEN:?set GW_COORDINATOR_TOKEN (see docs/testing.md §2) before provisioning}"
