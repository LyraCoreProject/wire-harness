...
# world logins via the `login-queue` wire-client probe (src/scenarios/probes.rs). It
...
#
# Usage: bash adapters/lyracore/test-login-queue.sh [N] [PASSWORD]
...
# The two binaries come from DIFFERENT repositories now (#245): the client is this harness's, the
# gateway is LyraCore's — this script needs the gateway only for its `provision` subcommand.
WC="${WIRE_BIN:-$HARNESS_DIR/target/debug/vanilla-wire}"
GW="$LYRACORE_DIR/target/debug/lyracore-gateway"
PREFIX="LQ"
...
if [[ ! -x "$GW" ]]; then
  ...
  cargo build -p lyracore-gateway --manifest-path "$LYRACORE_DIR/Cargo.toml" 2>&1
fi
