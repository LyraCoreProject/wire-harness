...
//! death/corpse-run -- the wire half of adapters/lyracore/test-scenario-*.sh.
...
pub(crate) fn try_dispatch(
    ...
    match mode {
        ...
        "vendor-sell" | "vendor-repair" | "vendor-buyback" => {
            vendor_sell_repair_buyback(mode, c, args)?
        }
...
fn scenario_quest(
    ...
    let giver: u64 = args
        ...
        .expect("usage: … scenario-quest <giver> <quest> <wolf1> <wolf2>");
    let quest: u32 = args
        .next()
        .and_then(|s| s.parse().ok())
        .expect("quest entry");
    ...
    c.send(&CMSG_QUESTGIVER_ACCEPT_QUEST {
        guid: Guid::new(giver),
        quest_id: quest,
    })?;
    ...
    for (i, wolf) in [(1u32, wolf1), (2u32, wolf2)] {
        ...
        match c.recv_for(std::time::Duration::from_secs(90), |m| match m {
            ...
            None => bail!(
                "STEP {} FAIL: no SMSG_QUESTUPDATE_ADD_KILL({i}/2) within 90s",
                2 + i
            ),
        }
        println!(
            "[scenario] STEP {} OK — SMSG_QUESTUPDATE_ADD_KILL {i}/2",
            2 + i
        );
    }
    ...
    let Some(gold) = gold else {
        bail!("STEP 5 FAIL: no SMSG_LOOT_RESPONSE within 5s of CMSG_LOOT")
    };
    if gold == 0 {
        bail!("STEP 5 FAIL: SMSG_LOOT_RESPONSE.gold == 0 (fixture wolves carry money)");
    }
    c.send(&CMSG_LOOT_MONEY {})?;
    c.send(&CMSG_LOOT_RELEASE {
        guid: Guid::new(wolf2),
    })?;
    println!("[scenario] STEP 5 OK — looted corpse (gold={gold}) + released");

    // STEP 6: complete -> the giver offers the reward.
    c.send(&CMSG_QUESTGIVER_COMPLETE_QUEST {
        guid: Guid::new(giver),
        quest_id: quest,
    })?;
    match c.recv_for(std::time::Duration::from_secs(5), |m| match m {
    ...
    c.send(&CMSG_QUESTGIVER_CHOOSE_REWARD {
        guid: Guid::new(giver),
        quest_id: quest,
        reward: 0,
    })?;
    ...
    println!(
        "[wire] SCENARIO-QUEST PASS \u{2713}  details->accept->2 kills->loot->offer->complete"
    );
    Ok(())
}
...
fn vendor_buy(
    ...
    println!(
        "[wire] VENDOR-BUY PASS \u{2713}  CMSG_BUY_ITEM({entry}) accepted (no SMSG_BUY_FAILED)"
    );
    Ok(())
}
...
fn equip_or_unequip(
    ...
    if equipping {
        c.send(&CMSG_AUTOEQUIP_ITEM {
            source_bag: 255,
            source_slot: slot,
        })?;
    } else {
        c.send(&CMSG_AUTOSTORE_BAG_ITEM {
            source_bag: 255,
            source_slot: slot,
            destination_bag: 255,
        })?;
    ...
    println!(
        "[wire] {} PASS \u{2713}  slot {slot} accepted (no INVENTORY_CHANGE_FAILURE)",
        if equipping {
            "EQUIP-FROM"
        } else {
            "UNEQUIP-FROM"
...
fn vendor_sell_repair_buyback(
    ...
    use wow_world_messages::vanilla::{
        BuybackSlot, CMSG_BUYBACK_ITEM, CMSG_REPAIR_ITEM, CMSG_SELL_ITEM,
    };
    let vendor: u64 = args
        .next()
        .and_then(|s| s.parse().ok())
    ...
    let arg: u64 = args
        .next()
        .and_then(|s| s.parse().ok())
        .expect("item guid / packet slot");
    match mode {
        "vendor-sell" => c.send(&CMSG_SELL_ITEM {
            vendor: Guid::new(vendor),
            item: Guid::new(arg),
            amount: 1,
        })?,
        "vendor-repair" => c.send(&CMSG_REPAIR_ITEM {
            npc: Guid::new(vendor),
            item: Guid::new(arg),
        })?,
        _ => c.send(&CMSG_BUYBACK_ITEM {
            guid: Guid::new(vendor),
            slot: BuybackSlot::try_from(arg as u32)
                .map_err(|_| anyhow!("bad buyback slot {arg}"))?,
        })?,
    }
    ...
    println!(
        "[wire] {} SENT \u{2713}  (state asserted via sql by the orchestrator)",
        mode.to_uppercase()
    );
    Ok(())
}
...
fn vendor_sell_buyback(
    ...
    signal_and_wait_consumed(
        ...
        &sold,
        30,
        "sell: orchestrator never confirmed the sell-state asserts",
    )?;
    println!("[scenario] SELL OK (orchestrator confirmed money + buyback ring)");
    c.send(&CMSG_BUYBACK_ITEM {
        guid: Guid::new(vendor),
        slot: BuybackSlot::try_from(69u32).unwrap(),
    })?;
    signal_and_wait_consumed(
        c,
        &bought,
        30,
        "buyback: orchestrator never confirmed the buyback-state asserts",
    )?;
...
fn scenario_train(
    ...
    c.send(&CMSG_TRAINER_LIST {
        guid: Guid::new(trainer),
    })?;
    let ids = c.recv_for(std::time::Duration::from_secs(5), |m| match m {
        Smsg::SMSG_TRAINER_LIST(t) => {
            Some(t.spells.iter().map(|sp| sp.spell).collect::<Vec<u32>>())
        }
        _ => None,
    });
    let Some(ids) = ids else {
        bail!("STEP 1 FAIL: no SMSG_TRAINER_LIST within 5s")
    };
    println!("[probe] SMSG_TRAINER_LIST spells={ids:?}");
    if !ids.contains(&spell) {
        bail!("STEP 1 FAIL: trainer list lacks {spell}");
    }
    ...
    let done = c.recv_for(std::time::Duration::from_secs(5), |m| {
        match m {
            Smsg::SMSG_TRAINER_BUY_SUCCEEDED(b) => {
                if b.id == spell {
                    ok_buy = true;
                }
            }
            Smsg::SMSG_TRAINER_BUY_FAILED(f) => {
                return Some(Err(anyhow!("STEP 2 FAIL: SMSG_TRAINER_BUY_FAILED {f:?}")))
            }
            Smsg::SMSG_LEARNED_SPELL(l) => {
                if l.id == spell {
                    ok_learn = true;
                }
            }
    ...
    let done = c.recv_for(std::time::Duration::from_secs(8), |m| {
        match m {
            ...
            Smsg::SMSG_CAST_RESULT(r) => {
                return Some(Err(anyhow!("STEP 3 FAIL: cast rejected: {r:?}")))
            }
            _ => {}
        }
    ...
    match done {
        ...
        Some(Err(e)) => return Err(e),
        None => {
            bail!("STEP 3 FAIL: START(timer={cast_ms})={started} GO={went} (want both) within 8s")
        }
    }
...
fn scenario_death(
    ...
    signal_and_wait_consumed(
        ...
        &ready,
        60,
        "STEP 1 FAIL: orchestrator never confirmed the death",
    )?;
    ...
    let Some(delay) = delay else {
        bail!("STEP 2 FAIL: no SMSG_CORPSE_RECLAIM_DELAY within 12s of CMSG_REPOP_REQUEST")
    };
    if delay != std::time::Duration::from_secs(30) {
        bail!("STEP 2 FAIL: reclaim delay {delay:?}, want 30s");
    }
    ...
    c.send(&CMSG_RECLAIM_CORPSE {
        guid: Guid::new(corpse),
    })?;
    ...
    signal_and_wait_consumed(
        ...
        &reclaimed,
        30,
        "STEP 4 FAIL: orchestrator never confirmed the resurrect",
    )?;
